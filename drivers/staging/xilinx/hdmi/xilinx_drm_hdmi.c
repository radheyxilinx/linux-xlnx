				   ...
				   struct drm_display_mode *adjusted_mode)
{
	...
	/* Reset current freq in phy clk detector - force frequency change */
#if 0	
	hdmi_dbg("Phy Curr Freq: %ld \n", XVphy_ClkDetGetRefClkFreqHz(VphyPtr, XVPHY_DIR_TX));
	hdmi_dbg("Phy Reset Tx Freq\n");	
	XVphy_ClkDetFreqReset(VphyPtr, 0, XVPHY_DIR_TX);
	hdmi_dbg("Phy Freq after reset: %ld \n", XVphy_ClkDetGetRefClkFreqHz(VphyPtr, XVPHY_DIR_TX));
	usleep_range(1000, 2000);
	hdmi_dbg("Phy Freq after delay: %ld \n", XVphy_ClkDetGetRefClkFreqHz(VphyPtr, XVPHY_DIR_TX));
#endif
...
static int xilinx_drm_hdmi_parse_of(struct xilinx_drm_hdmi *hdmi, XV_HdmiTxSs_Config *config)
{
	...
	if (rc < 0) {
		...
		config->Vtc.AbsAddr = vtc_config.BaseAddress = val;
	}
	...
	if (strcmp(format, "rgb") == 0) {
		...
		hdmi->xvidc_colorfmt = XVIDC_CSF_YCRCB_444;
...
static int xilinx_drm_hdmi_probe(struct platform_device *pdev)
{
	...
	/* Compute sub-core AbsAddres - Add subsystem base address to sub-core offset */
    config.HdmiTx.AbsAddr += config.BaseAddress;
	config.Vtc.AbsAddr += config.BaseAddress;
	if((config.HdmiTx.AbsAddr > config.HighAddress) ||
	   (config.Vtc.AbsAddr > config.HighAddress)) {
	   hdmi_dbg("sub-core address out-of range\n");
	   return -EFAULT; 
	}
	...
	if (IS_ERR(hdmi->clk)) {
		...
		hdmi_dbg("video-clk not ready - probe deferred\n");
		return ret;
	}
	...
	if (IS_ERR(hdmi->axi_lite_clk)) {
		...
		hdmi_dbg("axi-lite-clk not ready - probe deferred\n");
		return ret;
	}
	...
	if (IS_ERR(hdmi->retimer_clk)) {
		...
		hdmi_dbg("retimer-clk not ready - probe deferred\n");
		return ret;
