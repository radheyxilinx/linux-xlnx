					      ...
					      struct device_node *node)
{
	...
	if (i2c_slv && i2c_slv->dev.driver) {
		...
		if (!drm_i2c_driver) {
			DRM_ERROR("failed to initialize i2c slave, deferring\n");
			ret = -EPROBE_DEFER;
			goto err_out;
		}
		...
		if (!platform_slv) {
			DRM_DEBUG_KMS("failed to get an encoder slv, deferring\n");
			return ERR_PTR(-EPROBE_DEFER);
		}

		device_driver = platform_slv->dev.driver;
		if (!device_driver) {
			DRM_DEBUG_KMS("failed to get device driver, deferring\n");
			return ERR_PTR(-EPROBE_DEFER);
		}
		...
		if (!drm_platform_driver) {
			DRM_ERROR("failed to initialize platform slave, deferring\n");
			ret = -EPROBE_DEFER;
			goto err_out;
		}
